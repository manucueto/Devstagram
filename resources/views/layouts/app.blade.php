...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-100">
        <header class="p-5 border-b bg-white shadow">
            <div class="container mx-auto md:flex justify-between items-center my-4">
                <a href="{{route('home')}}" class=" text-3xl font-black sm:flex justify-center">
                    Devstagram
                </a href="{{route('home')}}">
                ...
                @auth
                    <nav class="flex gap-2 items-center">
                        <a  href="{{route('posts.create')}}" class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold cursor-pointer"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                          ...
                          Crear </a>
                        <a class="  text-gray-600 text-sm" href="{{route('posts.index', auth()->user()->username)}}">
                            Hola: 
                            <span class="font-bold">{{auth()->user()->username}}
                            </span>
                        </a>
                ...
                @guest
                <nav class="flex gap-2 items-center">
                    <a class=" uppercase text-gray-600 text-sm" href="{{route('login')}}">Loging</a>
        ...
        <footer class="text-center p-5 mt-10 text-gray-500 uppercase">
